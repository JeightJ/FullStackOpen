    ...
    if(window.confirm(`${newName} already exists in the phonebook, replace the old number with a new one?`)){
      personService
        ...
        .then(returnedPerson => {
          ...
          setNewName('')
          setNewNumber('')
          setSuccessMessage(`Updated ${returnedPerson.name}`) // Lisää tämä
          setTimeout(() => {
            setSuccessMessage(null) // Lisää tämä
          }, 5000)
        })
        .catch(error => {
...
const removePerson = (id) => {
  ...
  if (window.confirm(`Delete ${personToRemove.name} ?`)) {
    personService
      ...
      .then(() => {
        ...
        setSuccessMessage(`Removed ${personToRemove.name}`) // Lisää tämä
        setTimeout(() => {
          setSuccessMessage(null) // Lisää tämä
        }, 5000)
      })
  ...
  const personsToShow = filter
    ...
    return (
      <div>
        <h2>Phonebook</h2>
        <Notification.Notification message={errorMessage} />
